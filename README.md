...
Take the existing markup rendered from the `Stopwatch` component in `src/Stopwatch.js` and change into a fully functioning component. This means that you only have to add functionality -- not markup or styling -- to the existing code! You will need to add a state and the change the state using separate functions corresponding with their respective buttons. 
